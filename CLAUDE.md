...
`DBHttpClient` (`lib/app/core/http/db_http_client.dart`) wraps `dio` with a single `request(DBBaseHttpRequest request)` method and always returns `Result<DBError, Map<String, dynamic>>` — never `dynamic`. The dofusdu.de API returns a bare JSON array for search-style endpoints (no object envelope), so `DBHttpClient` wraps those under a `'data'` key before handing the body back (`{'data': [...]}`); a datasource for a list endpoint reads `responseBody['data'] as List<dynamic>` instead of casting the whole body to a list (see `ItemsRemoteDatasource.searchItems`). Object-shaped responses pass through unchanged. `DBError` (`lib/app/core/error/error.dart`) is a sealed class (`NetworkError`, `ApiError`, `UnknownError`) — its `message` is for logs only, never shown directly to the user. Pages map the `DBError` type to a localized string themselves (see `ItemSearchPage._failureMessage`). It's named `DBError` rather than `Error` specifically to avoid colliding with `dart:core`'s `Error` (programming-bug errors, not recoverable failures) everywhere it's imported alongside `Result`.
